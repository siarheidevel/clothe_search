...
class Sim_Swin_model(nn.Module):
    def __init__(self, hidden_dim=256, output_dim=192, dropout=0):
        super().__init__()
        self.backbone = timm.models.swin_transformer.swin_tiny_patch4_window7_224(pretrained=True)
        self.backbone_output_dim = 768
        # self.backbone = timm.models.swin_transformer.swin_base_patch4_window7_224_in22k(pretrained=True)
        ...
        self.head = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(self.backbone_output_dim, output_dim, bias=True),
            L2Norm()
        )
...
class TrainerSimilarity(pl.LightningModule):
    def __init__(self,args, label_count: int, category_count: int, category_weights,**kwargs):
        ...
        self.model = Sim_Swin_model(hidden_dim=args.hidden_dim, output_dim=args.output_dim, dropout=args.dropout)
    ...
    def configure_optimizers(self):       
        optimizer = torch.optim.AdamW([p for p in self.parameters() if p.requires_grad==True],
            lr=self.args.lr, weight_decay= 5e-5)
