...
def parse_args():
    ...
    parser.add_argument('--save_dir', default='./checkpoints/efnetb3s', type=str)
    parser.add_argument('--run_name', default='default_run', type=str)
    parser.add_argument('--log_parameters', default=0, type=int)
    parser.add_argument('--batch_size', default=6, type=int)
    parser.add_argument('--augm_count', default=3, type=int)
    parser.add_argument('--n_cpus', default=4, type=int)
    parser.add_argument('--output_dim', default=192, type=int)
...
def prepare_dataset(args):
    data_file = '/home/deeplab/devel/Clothes_search/util/customfashion_index_qanet_processed_limited_one_category_milti_seg.csv'
    ...
    category_gender_count = data.category_gender_id.max()+ 1
    inverted_weights_category_gender_count = data[['category_gender_id','category_gender_count']].drop_duplicates().sort_values(by='category_gender_id')['category_gender_count'].apply(lambda x:1/x).to_list()
    ...
    train_dataset = dataset.ClothingDataset(train_data, stage='train', augm_count=args.augm_count)
    ...
    train_dataloader = DataLoader(train_dataset,sampler=perclass_sampler, batch_size=args.batch_size, 
        num_workers=args.n_cpus ,prefetch_factor=3, pin_memory=True)
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size*2, shuffle=False,
        num_workers=args.n_cpus,prefetch_factor=3, pin_memory=True)
    return train_dataloader,val_dataloader, instance_count, category_gender_count, inverted_weights_category_gender_count
...
def run_model(args):
    ...
    train_dataloader,val_dataloader, instance_count, category_count, category_weights = prepare_dataset(args)

    model_trained = model.TrainerSimilarity(args, label_count=instance_count, category_count=category_count, category_weights=category_weights)
    # model_trained.load_model_from_checkpoint('/home/deeplab/devel/Clothes_search/exp5_limited_efnet/checkpoints/efnetb3s/epoch=2-step=17999.ckpt', strict=False)
    model_trained.load_model_from_checkpoint(checkpoint_path=args.save_dir+'/last.ckpt', strict=False)
    # init model weights
    ...
    every_10000_steps_checkpoint = ModelCheckpoint(
        dirpath=args.save_dir,
        every_n_train_steps =int(args.total_steps/10),
        filename ='{epoch}-{step}'
    )
    ...
    trainer = pl.Trainer.from_argparse_args(args,
        # resume_from_checkpoint='/home/deeplab/devel/Clothes_search/exp5_limited_efnet/checkpoints/efnetb3s/epoch=2-step=17999.ckpt',
