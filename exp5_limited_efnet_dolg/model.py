...
class LabelSmoothingCrossEntropyLoss(nn.Module):
    ...
    def forward(self, x, target):
        ...
        if self.weights is None:
            ...
            return (loss * self.weights[target]).mean() #https://discuss.pytorch.org/t/multi-label-multi-class-class-imbalance/37573/2
...
class MultiAtrous(nn.Module):
    ...
    def forward(self, x):
        ...
        local_feat.append(self.gap_branch(x).expand(local_feat[0].shape))
...
class Sim_DOLG_model(nn.Module):
    def __init__(self, hidden_dim=256, output_dim=128, dropout=0):
        super().__init__()
        self.backbone = EfficientNet.from_pretrained('efficientnet-b3')
        EfficientNet.get_image_size('efficientnet-b3')
        self.backbone.eval()
        # # freeze all
        for p in self.backbone.parameters():
            p.requires_grad=False
        
        # unfreeze last top blocks
        for b in self.backbone._blocks[-5:]:
            for m in b.modules():
                if not isinstance(m, torch.nn.BatchNorm2d):
                    for p in m.parameters():
                        p.requires_grad=True

        for m in list(self.backbone.modules())[-5:]:
            if not isinstance(m, torch.nn.BatchNorm2d):
                for p in m.parameters():
                    p.requires_grad=True

        # unfreez all
        for m in list(self.backbone.modules()):
            if not isinstance(m, torch.nn.BatchNorm2d):
                for p in m.parameters():
                    p.requires_grad=True
        ...
        self.backbone_output_dim = 1536
        self.output_dim = output_dim

        self.orthogonal_fusion = OrthogonalFusion()
        self.local_branch = DolgLocalBranch(136, hidden_dim)
        ...
        self.fc_2 = nn.Sequential(
            nn.Dropout(dropout),
            nn.Linear(int(2*hidden_dim), output_dim, bias=False)
        )
        ...
        #     nn.BatchNorm1d(self.backbone_output_dim),
        #     nn.Dropout(0.2),
        #     nn.Linear(self.backbone_output_dim, hidden_dim, bias=False),
        #     nn.LeakyReLU(0.1),
        #     nn.Linear(hidden_dim, output_dim, bias=False)
        # )
        self.local_branch.apply(utils.init_model_weights)
        self.fc_1.apply(utils.init_model_weights)
        self.fc_2.apply(utils.init_model_weights)


    def forward(self, x):
        output = self.backbone.extract_endpoints(x)

        local_feat = self.local_branch(output['reduction_4'])  # ,hidden_channel,16,16
        global_feat = self.fc_1(self.gem_pool(output['reduction_6']).squeeze())  # ,1024

        feat = self.orthogonal_fusion(local_feat, global_feat)
        feat = self.gap(feat).squeeze()
        feat = self.fc_2(feat)
        out = F.normalize(feat, p=2, dim=1)
        return out
...
class TrainerSimilarity(pl.LightningModule):
    def __init__(self,args, label_count: int, category_count: int, category_weights,**kwargs):
        ...
        self.model = Sim_DOLG_model(hidden_dim=args.hidden_dim, output_dim=args.output_dim, dropout=args.dropout)
        ...
        self.category_loss = LabelSmoothingCrossEntropyLoss(smoothing=0.1, temperature=0.5, weights=category_weights)
        ...
        self.example_input_array = torch.empty(4, 3, 300, 300)
    ...
    def configure_optimizers(self):       
        optimizer = torch.optim.Adam([p for p in self.parameters() if p.requires_grad==True],
            lr=self.args.lr)
        ...
        lr_scheduler = pl_bolts.optimizers.lr_scheduler.LinearWarmupCosineAnnealingLR(
            optimizer, warmup_epochs=self.args.warmup_steps, max_epochs=self.args.total_steps, warmup_start_lr=0, eta_min=0.0, last_epoch=-1)
    ...
    def load_model_from_checkpoint(self, checkpoint_path: str, strict:bool = True):
