...
class Sim_model(nn.Module):
    def __init__(self, output_dim=64):
        ...
        self.pooling_max = nn.AdaptiveMaxPool2d(1)
        self.pooling_avg = nn.AdaptiveAvgPool2d(1)

        self.head_gem = nn.Linear(self.backbone_output_dim, output_dim, bias=False)
        self.head_gem.apply(utils.init_model_weights)
        self.head_max = nn.Linear(self.backbone_output_dim, output_dim, bias=False)
        self.head_max.apply(utils.init_model_weights)
        self.head_avg = nn.Linear(self.backbone_output_dim, output_dim, bias=False)
        self.head_avg.apply(utils.init_model_weights)
    ...
    def forward(self, x):
        ...
        gem_out = self.head_gem(gem_raw)
        max_out = self.head_max(self.pooling_max(out).view(n,-1))
        avg_out = self.head_avg(self.pooling_avg(out).view(n,-1))
        out = torch.cat((F.normalize(gem_out, p=2, dim=1), F.normalize(max_out, p=2, dim=1), F.normalize(avg_out, p=2, dim=1)), dim=-1)
        out = F.normalize(out, p=2, dim=1)
        return out, gem_raw


class TrainerSimilarity(pl.LightningModule):
    def __init__(self,args, label_count: int, category_count: int, gender_count:int, **kwargs):
        super().__init__()
        self.args = args
        self.holder = {} #defaultdict(list)
        self.model = Sim_model(output_dim=args.output_dim)

        self.auxiliary_module = nn.Sequential(
            nn.BatchNorm1d(self.model.backbone_output_dim),
            nn.Linear(self.model.backbone_output_dim, label_count, bias=True))
        ...
        self.category_classifier = nn.Sequential(
            nn.BatchNorm1d(self.model.backbone_output_dim),
            nn.Linear(self.model.backbone_output_dim, category_count, bias=True))
        self.category_classifier.apply(utils.init_model_weights)
        self.category_loss = LabelSmoothingCrossEntropyLoss(smoothing=0.2, temperature=0.5)

        self.gender_classifier = nn.Sequential(
            nn.BatchNorm1d(self.model.backbone_output_dim),
            nn.Linear(self.model.backbone_output_dim, gender_count, bias=True))
        self.gender_classifier.apply(utils.init_model_weights)
        self.gender_loss = LabelSmoothingCrossEntropyLoss(smoothing=0.2, temperature=0.5)
        ...
        # loss = losses.ContrastiveLoss(pos_margin=0.05, neg_margin = 0.2, distance=distance)
        # miner = miners.PairMarginMiner(pos_margin=0.05, neg_margin = 0.2, distance=distance)
        ...
        self.loss_func = losses.CrossBatchMemory(
            loss=loss,
            embedding_size=3*args.output_dim, memory_size=4096,miner=miner)
        # self.miner_func = miner
        # self.loss_func = loss


    def forward(self, x):
        emb, descriptor = self.model(x)
        return emb, descriptor


    def training_step(self, batch, batch_idx):
        # training_step defined the train loop.
        # garment_tensor, row.label_id, row.category_id, row.gender_id,  augm_tensors , idx
        garment_tensor, label_id, category_id, gender_id , augm_tensors, idx = batch
        embeddings, descriptors = self.forward(garment_tensor)

        if augm_tensors is not None and len(augm_tensors)>0:
            init_label_id,init_category_id,init_gender_id, init_idx = label_id, category_id, gender_id, idx
            for i in range(len(augm_tensors)):
                augm_emb, augm_descriptor = self.forward(augm_tensors[i])
                embeddings = torch.cat((embeddings, augm_emb), 0)
                descriptors = torch.cat((descriptors, augm_descriptor), 0)
                label_id = torch.cat((label_id,init_label_id),0)
                category_id = torch.cat((category_id,init_category_id),0)
                gender_id = torch.cat((gender_id,init_gender_id),0)
        ...
        aux_logits = self.auxiliary_module(descriptors)
        aux_loss = self.auxilary_loss(aux_logits, label_id)
        category_logits = self.category_classifier(descriptors)
        category_loss  = self.category_loss(category_logits, category_id)
        gender_logits = self.gender_classifier(descriptors)
        gender_loss  = self.gender_loss(gender_logits, gender_id)

        dml_loss = self.loss_func(embeddings, label_id)
        loss = dml_loss + self.args.category_weight * aux_loss + self.args.gender_weight*category_loss + self.args.gender_weight*gender_loss
        ...
        self.log("train_gender_loss", gender_loss.item())
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        garment_tensor, label_id, category_id, gender_id , _, idx = batch
        embeddings, descriptors = self.forward(garment_tensor)
        ...
        # Logging to TensorBoard by default
        self.log("val_category_loss", aux_loss.item())
        ...
        self.log("val_gender_loss", gender_loss.item())
