...
class TrainerSimilarity(pl.LightningModule):
    def __init__(self,args, label_count: int, category_count: int, gender_count:int, **kwargs):
        ...
        self.gender_classifier = nn.Sequential(
            nn.BatchNorm1d(self.model.output_dim),
            nn.Linear(self.model.output_dim, gender_count, bias=True))
        ...
        self.loss_func = losses.CrossBatchMemory(
            loss=loss,
            embedding_size=args.output_dim, memory_size=4096,miner=miner)
    ...
    def training_step(self, batch, batch_idx):
        ...
        embeddings, _ = self.forward(garment_tensor)
        ...
        if augm_tensors is not None and len(augm_tensors)>0:
            ...
            for i in range(len(augm_tensors)):
                augm_emb, _ = self.forward(augm_tensors[i])
        ...
        loss = dml_loss + self.args.category_weight * category_loss + self.args.gender_weight*gender_loss
