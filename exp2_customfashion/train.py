...
def parse_args():
    ...
    parser.add_argument('--gender_weight', default=0.1, type=float) 
...
def prepare_dataset(args):
    data_file = '/home/deeplab/devel/Clothes_search/util/customfashion_index_qanet_processed.csv'
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=args.batch_size*3, shuffle=False,
        num_workers=args.n_cpus,prefetch_factor=10, pin_memory=True)
    return train_dataloader,val_dataloader, category_count, gender_count
...
def run_model(args):
    ...
    train_dataloader,val_dataloader, category_count, gender_count = prepare_dataset(args)
    ...
    trainer = pl.Trainer.from_argparse_args(args,
        resume_from_checkpoint='/home/deeplab/devel/Clothes_search/exp2_customfashion/checkpoints/dense121_category/epoch=4-step=329999.ckpt',
        # track_grad_norm=2,weights_summary="full",
        callbacks=[every_10000_steps_checkpoint,last_checkpoint,every_10_epoch_checkpoint],
        # resume_from_checkpoint=args.save_dir+'/last.ckpt',
        # fast_dev_run=1,
        gpus = [CUDA_DEVICE_ID],
        max_epochs=20,
        )
    model_trained = model.TrainerSimilarity(args, category_count=category_count, gender_count=gender_count)
